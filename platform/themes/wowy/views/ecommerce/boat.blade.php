                                ...
                                <div class="col-4">
                                    ...
                                    <div class="currency-dropdown dropdown">
                                        <button class="dropdown-toggle" type="button" id="currencyDropdown"
                                            ...
                                            {{ session('currency', 'Select Currency') }}
                                        </button>
                                                ...
                                                @if ($currency->id !== get_application_currency_id())
                                                    <li>
                                                        <a class="dropdown-item currency-item"
                                                            ...
                                                            data-currency-title="{{ $currency->title }}">{{ $currency->title }}</a>
                                                    </li>
                                                @endif
                                            @endforeach
                                        </ul>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        optionInputs = document.querySelectorAll('input[name^="option["]');
        const boatPriceInput = document.querySelector('input[name="boat_price"]');

        currencyItems.forEach(item => {
            item.addEventListener('click', function(e) {
                e.preventDefault();
                const currencyCode = this.dataset.currencyTitle;
                const baseUrl = "<?php echo url('/'); ?>";

                const optionIds = Array.from(optionInputs)
                    .map(input => input.value)
                    .filter(value => !isNaN(value) && value !== "")
                    .map(value => parseInt(value, 10));

                const boatPrice = parseFloat(boatPriceInput.value);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            console.error('Currency conversion failed:', response
                                .error);
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('Currency conversion request failed:', error);
                    }
                });
            });
        });
        ...
        optionInputs.forEach(input => {
            ...
            if (prices[id]) {
                ...
                if (input.closest('.form-check')) {
                    // For toggle layout
                    priceElement = input.closest('.form-check').querySelector('.tog');
                } else {
                    // For radio layout
                    priceElement = input.nextElementSibling.querySelector('.color-name');
                }

                if (priceElement) {
                    const currentText = priceElement.textContent;
                    const newPrice = prices[id];
                    // Replace only the price part
                    priceElement.textContent = currentText.replace(/\(.*?\)/, `(${newPrice})`);
                }
            }
        });
    }
    ...
    function updateTotals(totals) {
        document.querySelector('.sub-total').textContent = totals.subTotal;
        document.querySelector('.vat-price').textContent = totals.vat;
        document.querySelector('.vat-total').textContent = totals.total;
    }
