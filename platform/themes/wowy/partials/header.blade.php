...
<div class="section hero mob">
    <img class="w-100 h-100"
         src="{{ RvMedia::getImageUrl(isset($page) && isset($page->image) ? $page->image : 'bannerhome.jpg','400 x 400')}}"
         alt="Ocean Boats Banner Image">
            ...
            <div class="header-bar-container">
                ...
                <div class="header-thin">
                    <ul class="header-navbar-nav">
                        ...
                        @endif
                        <li>
                            <form id="standard-3" action="{{ route('public.products') }}" id="form2">
        ...
        <div id="header-bottom1">
            ...
            <ul class="header-navbar-bottom-right header-navbar-bottom">
                <div class="header-action-2">
                    <div class="header-action-icon-2">
                        <a href="{{ route('public.wishlist') }}" class="wishlist-count">
                            ...
                            <span class="pro-count blue">@if (auth('customer')->check())
                ...
                @if (auth('customer')->check())
                    <li><a href="{{ route('customer.overview') }}"><img alt="{{ __('Sign In') }}"
                                                                        src="{{ Theme::asset()->url('images/icons/icon-user-white.svg') }}"
                                                                        style="width: 18%;position: absolute;float: right;"></a>
                    </li>
                @else
                    <li><a href="{{ route('customer.login') }}">
                            <button type="button" class="btn login">Log In</button></li>
                @endif
            ...
            <div class="thin-header">
                ...
                <ul class="header-navbar-nav-right header-navbar-nav">
                    @if (theme_option('social_links'))
                        @foreach(json_decode(theme_option('social_links'), true) as $socialLink)
                            <li><a href="{{ $socialLink[2]['value'] }}"><i
                                            class="{{ $socialLink[1]['value'] }}"></i></a></li>
                        @endforeach
                    @endif
                </ul>
                        ...
                        <ul>
                            ...
                            @if (is_plugin_active('ecommerce') && EcommerceHelper::isOrderTrackingEnabled())
                                <li><i class="far fa-anchor mr-5"></i><a
                                            href="{{ route('public.orders.tracking') }}">{{ __('Track Your Order') }}</a>
                                </li>
                            @endif
                        </ul>
                    </div>
                        ...
                        <div class="header-info header-info-right">
                            <ul>
                                ...
                                @if (is_plugin_active('ecommerce'))
                                    ...
                                    @else
                                        <li><a href="{{ route('customer.login') }}">{{ __('Log In / Sign Up') }}</a>
                                        </li>
                                    @endif
                                @endif
                            </ul>
                        ...
                        <div class="header-action-2">
                                ...
                                <div class="header-action-icon-2">
                                    <a href="{{ route('public.compare') }}" class="compare-count">
                                        <img class="svgInject" alt="{{ __('Compare') }}"
                                             src="{{ Theme::asset()->url('images/icons/icon-compare.svg') }}">
                            ...
                            @if (EcommerceHelper::isWishlistEnabled())
                                <div class="header-action-icon-2">
                                    <a href="{{ route('public.wishlist') }}" class="wishlist-count">
                                        <img class="svgInject" alt="{{ __('Wishlist') }}"
                                             src="{{ Theme::asset()->url('images/icons/icon-heart.svg') }}">
                                        <span class="pro-count blue">@if (auth('customer')->check())
                                                <span>{{ auth('customer')->user()->wishlist()->count() }}</span>
                                            @else
                                                <span>{{ Cart::instance('wishlist')->count() }}</span>
                            ...
                            <div class="header-action-icon-2">
                                <a href="{{ route('customer.login') }}">
                                    <img alt="{{ __('Sign In') }}"
                                         src="{{ Theme::asset()->url('images/icons/icon-user.svg') }}">
                                </a>
                            </div>
                        </div>
                    </div>
